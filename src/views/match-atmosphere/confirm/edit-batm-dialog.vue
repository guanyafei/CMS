<template>
  <sn-confirm title="编辑B面氛围" txt :flag="matchFlagBEdit">
...
export default {
  ...
  methods: {
        ...
        this.$ajax({
          ...
          success: res => {
            if (res.retCode == '0') {
              ...
              recommendedMatchColor = res.data.recommendedMatchColor;
              searchBarColor = res.data.searchBarColor;
              topTitleColor = res.data.topTitleColor;
              let recommendedMatchColorNew = JSON.parse(recommendedMatchColor);
              let topTitleColorNew = JSON.parse(topTitleColor);
              let searchBarColorNew = JSON.parse(searchBarColor);
              if (recommendedMatchColorNew) {
                this.fontColors.remMatchBg = recommendedMatchColorNew.background;
                this.fontColors.remMatchTxtBig = recommendedMatchColorNew.bigText;
                this.fontColors.remMatchTxtSmall = recommendedMatchColorNew.smallText;
                this.fontColors.txtLoading = recommendedMatchColorNew.loading;
              }
              if (topTitleColorNew) {
              ...
              if (searchBarColorNew) {
                ...
                this.fontColors.searchIcon = searchBarColorNew.searchIcon;
              }
            } else {
    ...
    revoke() {
      this.$ajax({
        url: DI.atmosphere.faceBRevoke,
        data: JSON.stringify({ skinId: this.skinId }),
        context: this,
        type: 'post',
        loadingText: '正在查询，请稍候！',
        success: res => {
      ...
      this.$ajax({
        ...
        data: JSON.stringify({
          ...
          recommendedMatchColor:
            '{"background":' +
            JSON.stringify(this.fontColors.remMatchBg.hex ? this.fontColors.remMatchBg.hex : this.fontColors.remMatchBg) +
            ',"bigText":' +
            JSON.stringify(this.fontColors.remMatchTxtBig.hex ? this.fontColors.remMatchTxtBig.hex : this.fontColors.remMatchTxtBig) +
            ',"smallText":' +
            JSON.stringify(this.fontColors.remMatchTxtSmall.hex ? this.fontColors.remMatchTxtSmall.hex : this.fontColors.remMatchTxtSmall) +
            ',"loading":' +
            JSON.stringify(this.fontColors.txtLoading.hex ? this.fontColors.txtLoading.hex : this.fontColors.txtLoading) +
            '}',
          searchBarColor:
            '{"background":' +
            ...
            ',"searchIcon":' +
            JSON.stringify(this.fontColors.searchIcon.hex ? this.fontColors.searchIcon.hex : this.fontColors.searchIcon) +
            '}',
          topTitleColor:
            '{"selectedColor":' +
    ...
    cancelBtn() {
      ...
      this.$emit('update:matchFlagBEdit', false);
