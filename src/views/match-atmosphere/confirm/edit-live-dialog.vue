<template>
  <sn-confirm title="编辑直播浮层" txt :flag="matchLiveFlagEdit">
    <sn-form ref="commonFloor" :model="editFloorInfo" :rules="floorRules" label-width="140px" formWidth="600">
      ...
      <sn-form-item label="跳转类型" prop="resource">
        <sn-input v-model="jumpTypeTxt" width="410" disabled></sn-input>
      </sn-form-item>
      <sn-form-item v-if="jumpUrlShow" label="URL地址" prop="resource">
        <sn-input v-model="jumpUrl" placeholder="请输入URL地址" width="410" @blur="checkUrl" disabled></sn-input>
      </sn-form-item>
      <sn-form-item v-if="programShow" label="节目ID" prop="resource">
        <sn-input v-model="matchitemShowId" placeholder="请输入节目ID" inputType="number" width="410" @blur="quyeryMatchitem" disabled></sn-input>
      </sn-form-item>
...
export default {
  name: 'EditLiveDialog',
  props: ['matchLiveFlagEdit', 'skinId'],
  components: {
    ActName,
    TimeRange
  },
  ...
  methods: {
    ...
    cancelBtn() {
      ...
      this.$nextTick().then(() => {
        ...
        this.$emit('update:matchLiveFlagEdit', false);
    ...
    getDetail() {
      if (this.skinId) {
        this.$ajax({
          ...
          success: res => {
            if (res.retCode == '0') {
              this.jumpType = res.data.jumpType;
              this.endTime = res.data.endTime;
              this.jumpUrl = res.data.jumpUrl;
              this.startTime = res.data.startTime;
              this.matchItemInfo.matchDes = res.data.matchInfo;
              this.matchitemShowId = res.data.programId;
              this.skinDownloadUrl = res.data.skinDownloadUrl;
              this.matchItemInfo.matchTime = res.data.matchTime;
