<template>
  <div class="container">
      ...
      <section class="form-content">
        <sn-form :model="formData" ref="formData" label-width="140px">
          <sn-form-item :label="showData.contentTitle" prop="contentTitle" :rules="titleRules">
            <sn-input :placeholder="`请输入${showData.contentTitle}`" :maxlength="30" :totalWords="30" v-model="formData.contentTitle" width="640" :showWord="true"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="getStyle()" label="展示样式" prop="isBigImg" :rules="isBigImgRules">
            <sn-select v-model="formData.isBigImg" placeholder="请选择" radius="16" width="120" @change="handleStyle">
              <sn-option v-for="item in styleList" :key="item.id" :name="item.name" :value="item.value"></sn-option>
            </sn-select>
          </sn-form-item>
          <template v-if="contentTypeItem.key!='set'&&contentTypeItem.key!='topic'&&contentTypeItem.key!='program'&&contentTypeItem.key!='column'&&contentTypeItem.key!='match'&&contentTypeItem.key!='group'">
            <sn-form-item v-if="formData.isBigImg!==2" :label="showData.contentCover" prop="contentCover" :rules="coverRules">
              <sn-cropper v-model="formData.contentCover" :aspectRatio="aspectRatio" :height="aspectRatioHeight" width="500" ref="cropper" label="封面" :lib-imglist="formData.listGroup" :nolibrary="false" :isScoped="!showData.disValidCoverPixel"></sn-cropper>
            </sn-form-item>
            <!-- 三图 -->
            <sn-form-item v-show="formData.isBigImg==2" label="资讯封面" ref="imgList" :rules="threeCoverRules">
              <sn-cropper-three :listGroup="formData.listGroup" :imgList="formData.imgList" :newsId="contentItem.contentId" @img-group="imgGroupChange" :aspectRatio="aspectRatio" aspectWith="255" :aspectHeight="255*148/226+''" width="200" :height="200*148/226+''" ref="cropper-three" label="封面" :nolibrary="contentItem.contentType=='11'"></sn-cropper-three>
              <div class="form-item__error" v-if="imgGroupFlag">请上传图片</div>
            </sn-form-item>
          </template>
          <!--比赛编辑-->
          <templete v-if="contentTypeItem.key == 'match'">

          </templete>
          <!--小组编辑-->
          <templete v-if="contentTypeItem.key == 'group'">
          ...
          <sn-form-item label="引导语" v-if="contentItem.contentType=='6' || contentItem.contentType=='13' || contentItem.contentType=='14' || contentItem.contentType=='15' || contentItem.contentType=='16'" prop="contentAttachment" :rules="contentAttachmentRules">
            <sn-input width="429" placeholder="请输入引导语" :maxlength="30" :totalWords="30" :showWord="true" v-model="formData.contentAttachment"></sn-input>
          </sn-form-item>
        </sn-form>
      </section>
      <div class="btns">
    ...
    <sn-confirm title="提示" :flag="checkWordConfirm" @close="close" @sure="sure" confirmButtonText="继续保存" cancelButtonText="返回检查" close-icon noflag>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      formData: {
        contentAttachment: contentItem.tips || '',
        contentTitle: '',
        contentCover: '',
        showLabel: '',
        isBigImg: 0,
  ...
  computed: {
    coverArr() {
      const { contentCover } = this.contentItem;
    ...
    titleRules() {
      if (
        this.contentItem.contentType == '6' ||
        this.contentItem.contentType == '13' ||
        this.contentItem.contentType == '14' ||
        this.contentItem.contentType == '15' ||
        this.contentItem.contentType == '16'
      ) {
        return [{
            min: 1,
            max: 30,
            required: true,
            message: `请输入${this.showData.contentTitle},长度在1到30个字符`,
            trigger: 'change'
    ...
    coverRules() {
      let singleCoverValidator = (rule, value, callback) => {
        if (this.showData.disValidCoverPixel || this.isGifUrl(value)) {
          ...
          getImgPixelSize(value, this.formData.isBigImg == 1, { max: null, min: 500 })
            .then(() => {
              callback();
            })
            .catch(err => {
              if (err.min && err.width < err.min) {
                callback(new Error(`当前封面的像素宽度${err.width}px,小于${err.min}px`));
              }
            });
        }
      };
  ...
  methods: {
    ...
    getStyle() {
      return !(
        this.contentItem.contentType == '5' ||
        this.contentItem.contentType == '6' ||
        this.contentItem.contentType == '7' ||
        this.contentItem.contentType == '16' ||
        this.contentItem.contentType == '13' ||
        this.contentItem.contentType == '14' ||
        this.contentItem.contentType == '15'
      ...
      if (this.formData.isBigImg != val) {
        ...
        if (val == 2) {
          if (this.contentItem.contentType && ['1', '2', '3', '8', '9', '10'].indexOf('' + this.contentItem.contentType) === -1) {
            // tempList = this.formData.imgList;
          } else {
            this.formData.imgList = {
              img1: tempList[0] || '',
              img2: tempList[1] || '',
          ...
          this.formData.contentCover = this.contentItem.contentCover.split(';')[0];
        }
      }
      this.imgGroupFlag = false;
    },
    ...
    saveAjax() {
      ...
      Promise.all(getImgWidthPromises).then(arr => {
        submitData.imageWidths = arr.reduce((str, item) => {
          if (str) {
            str += `,${item.width}`;
          } else {
            str = `${item.width}`;
        ...
        params.imageWidths = submitData.imageWidths;
        params.contentCover = this.formData.isBigImg == 2 ? this.handleImgList(this.formData.imgList) : submitData.contentCover;
        ...
        if (
          ...
          params.tips = submitData.contentAttachment;
        }
        params = this.$bus.deleteNullProperty(params);
        this.$ajax({
          url: DI.channel.modifyContent,
