<template>
  <div class="container">
    ...
    <sn-topbar class="title" :title="getTitle">
      <span slot="subtitle">信息流ID: {{infoFlow.infoFlowId}}</span>
      <span slot="link">
        <a href="javascript:;" v-if="infoFlow.subjectType == '3'" @click="handleColumn">栏目管理</a>
      </span>
    </sn-topbar>
    <div>
        ...
        <div class="search-box-topic">
          ...
          <div class="select-box" v-if="contentType == '0'">
            ...
            <sn-select v-model="newsType" placeholder="全部" radius="16" width="120" @change="handleNewsType">
              ...
              <sn-option v-for="item in newsTypeList" :key="item.key" :name="item.name" :value="item.value" :disabled="item.disabled"></sn-option>
            </sn-select>
          </div>
      ...
      <div>
        ...
        <sn-datatable :data="tableList" :border="true" ref='table'>
          ...
          <sn-datacolumn v-if="infoFlow.subjectType == '3'" label="所属栏目" prop="programa" width="8%">
          </sn-datacolumn>
          ...
          <template slot-scope="{row}" slot="info">
            <div class="adv-info">
              <div class="img-wrap">
                ...
                <img class="adv-img" :src="handleCover(row)" onerror="javascript:this.src='';" alt="暂无图片" />
          ...
          <template slot-scope="{row}" slot="programa">
            {{row.columnName}}
          </template>
          ...
          <template slot-scope="{row}" slot="opt">
            <div class="is-column">
              ...
              <template v-if="infoFlow.subjectType == '3'">
    ...
    <!-- 删除框 -->
    <sn-confirm txt ref="delConfirm" :flag="delConFlag" @sure="delContent(delContentObj)" @close="hideDelConfirm">
      确定要删除吗？
    </sn-confirm>
    <!-- 批量删除框 -->
    ...
    <sn-confirm @sure="sure('ruleForm')" @close="close" txt :flag="insertFlag" title="插入内容">
      <sn-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="80px" formWidth="480">
        ...
        <templete>
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && (ruleForm.type == '0' || ruleForm.type == '9')" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
          ...
          <sn-form-item v-if="ruleForm.style!==2 && (ruleForm.type == '0' || ruleForm.type == '9')" label="资讯封面" prop="img" ref="img">
            <sn-cropper v-model="ruleForm.img" height="131" :width="aspectRatioWidth" :aspectRatio="aspectRatio" ref="cropper" label="封面" isScoped :lib-imglist="ruleForm.listGroup" :nolibrary="false"></sn-cropper>
          </sn-form-item>
          <!-- 三图 -->
          <sn-form-item v-if="ruleForm.type == '0' || ruleForm.type == '9'" v-show="ruleForm.style==2" label="资讯封面" prop="imgList" ref="imgList">
            <sn-cropper-three :aspectRatio="226/148" :newsId="ruleForm.id" :listGroup="ruleForm.listGroup" :imgList="ruleForm.imgList" ref="cropper-three" label="封面" aspectWith="255" :aspectHeight="255*148/226+''" width="130" :height="130*148/226+''" @img-group="imgGroupChange"></sn-cropper-three>
        ...
        <templete>
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '4'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
          ...
          <sn-form-item v-if="ruleForm.type == '4'" label="视频封面" prop="img">
            <sn-cropper v-model="ruleForm.img" height="131" :width="aspectRatioWidth" :aspectRatio="aspectRatio" ref="cropper" label="封面"></sn-cropper>
          </sn-form-item>
        </templete>
        <!--视频合集-->
        <templete>
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '5'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
        ...
        <templete>
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '6'" label="所属栏目" prop="colType">
            <sn-select v-if="ruleForm.type == '6'" v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
              <div v-if="duringAjaxColType" class="text-center">数据加载中。。。</div>
          ...
          <sn-form-item v-if="ruleForm.type == '6'" label="引导语" prop="contentAttachment" :rules="guideLanguageRules">
            <sn-input width="100%" placeholder="请输入引导语" :maxlength="30" :totalWords="30" :showWord="true" v-model="ruleForm.contentAttachment"></sn-input>
          </sn-form-item>
        </templete>
        <!--帖子-->
        <templete>
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '7'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
        ...
        <template>
          <sn-form-item v-if="ruleForm.type == '11'" label="展示文字" prop="title">
            <sn-input width="100%" placeholder="请输入链接展示文字，5-30个字" :maxlength="30" :totalWords="30" v-model="ruleForm.title" :showWord="true"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '11'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
          ...
          <sn-form-item label='跳转链接' ref="formItem" prop="jumpUrl" :rules="linkJumpRules" v-if="ruleForm.type == '11'">
            <sn-radio-group v-model="ruleForm.jumpStyle" @change="changeJumpStyle">
              <sn-radio v-for="linkType in linkTypes" :key="linkType.key" :label="linkType.value">{{ linkType.name }}</sn-radio>
            </sn-radio-group>
            <sn-input v-model="ruleForm.jumpUrl" :placeholder="jumpPlaceholder" width="400"></sn-input>
          </sn-form-item>
          ...
          <sn-form-item v-if="ruleForm.type == '11'" v-show="ruleForm.style==2" label="资讯封面" prop="imgList" ref="imgList">
            <sn-cropper-three :aspectRatio="226/148" :newsId="ruleForm.id" :listGroup="ruleForm.listGroup" :imgList="ruleForm.imgList" ref="cropper-three" label="封面" aspectWith="255" :aspectHeight="255*148/226+''" width="120" :height="120*148/226+''" @img-group="imgGroupChange"></sn-cropper-three>
        ...
        <templete>
          <sn-form-item v-if="ruleForm.type == '13' || ruleForm.type == '16'" :label="getRuleFormTypeName(ruleForm.type) + '查询'" prop="id" class="is-required">
            ...
            <sn-autocomplete v-model="ruleForm.program_column_id" valueKey="matchName" :fetchSuggestions="queryProgramAndColumnAsync" :resetFun="resetFun" :isSuggestionsPages="true" :placeholder="'请输入' + getRuleFormTypeName(ruleForm.type) + 'ID或标题'" @select="selectProgramAndColumn" :width="400" :resetFlag="resetFlag" ref="autoComplete">
          ...
          <sn-form-item v-if="ruleForm.type == '13' || ruleForm.type == '16'" :label="getRuleFormTypeName(ruleForm.type) + '标题'" prop="title" :rules="[{
            ...
            <sn-input width="100%" :placeholder="'请输入' + getRuleFormTypeName(ruleForm.type) + '标题'" :maxlength="30" :totalWords="30" :showWord="true" v-model="ruleForm.title"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="ruleForm.type == '13' || ruleForm.type == '16'" label="引导语" prop="contentAttachment" :rules="guideLanguageRules">
            <sn-input width="100%" placeholder="请输入引导语" :maxlength="30" :totalWords="30" :showWord="true" v-model="ruleForm.contentAttachment"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="infoFlow.subjectType=='3' && (ruleForm.type == '13' || ruleForm.type == '16')" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
        ...
        <templete>
          <sn-form-item v-if="ruleForm.type == '15'" label="选择比赛" prop="matchLabels" :rules="matchRules" ref="matchLabels">
            <sn-autocomplete v-model="ruleForm.searchInput" valueKey="matchName" :fetchSuggestions="querySearchAsync" :isSuggestionsPages="true" placeholder="输入球队/比赛关键词快速添加" @select="selectItem" :width="400">
          ...
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '15'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
        ...
        <templete>
          ...
          <sn-form-item v-if="ruleForm.type == '14'" label="小组名称" prop="groupName">
            <sn-input width="100%" placeholder="请输入小组名称" :maxlength="30" :totalWords="30" :showWord="true" v-model="ruleForm.groupName" :disabled="!ruleForm.groupId"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="ruleForm.type == '14'" label="引导语" prop="contentAttachment" :rules="guideLanguageRules">
            <sn-input width="100%" placeholder="请输入引导语" :maxlength="30" :totalWords="30" :showWord="true" v-model="ruleForm.contentAttachment"></sn-input>
          </sn-form-item>
          <sn-form-item v-if="infoFlow.subjectType=='3' && ruleForm.type == '14'" label="所属栏目" prop="colType">
            <sn-select v-model="ruleForm.colType" width="100%" @change="handleSelectInsertColumn">
              <template v-if="!duringAjaxColType">
                <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item.id" :disabled="item.disabled"></sn-option>
              </template>
          ...
          <sn-form-item v-if="ruleForm.type == '14'" label="楼层展示内容数量" labelWidth="145px" prop="showNum">
            <sn-radio-group v-model="showNumColumn">
              <sn-radio v-for="(item,index) in showNumColumnList" :label="index" :key="index">{{item.labelName}}</sn-radio>
            </sn-radio-group>
          </sn-form-item>
        </templete>
      </sn-form>
    </sn-confirm>
    <sn-confirm title="提示" :flag="checkWordConfirm" close-icon @close="closeCheck" @sure="sureCheck" confirmButtonText="继续保存" cancelButtonText="返回检查" noflag>
    ...
    <!--添加到栏目弹框-->
    <sn-confirm txt title="添加到栏目" ref="addColumnConfirm" :flag="addColumnFlag" @sure="addColSure('formData')" @close="addColClose" closeIcon>
      <sn-form ref="formData" :model="formData" :rules="columnRuleForm" label-width="86px" formWidth="286">
        <sn-form-item label="栏目名称:" prop="columnName">
          <sn-select v-model="formData.columnName" width="180" @change="handleSelectAddColumn">
            <sn-option v-for="item in columnTypeList" :key="item.id" :name="item.columnName" :value="item"></sn-option>
          </sn-select>
        </sn-form-item>
      </sn-form>
    </sn-confirm>
    <!-- 撤销栏目弹窗 -->
    <sn-confirm txt title="撤销栏目" ref="removeColumnConfirm" :flag="removeColumnFlag" @sure="removeColumnSure" @close="removeColumnClose">
      确定要撤销吗？
    </sn-confirm>
    <!--内容来源-匹配-->
    <!--<Tag ref="tag" v-show="true"/>-->
  </div>
</template>
...
import MatchTag from 'widgets/infoCommon/infoEditDetails/tags/match';
...
export default {
  ...
  data() {
    let contentTypeList = Constant.CONTENT_LIST_TYPES;
    let defaultType = '0';
    if (this.channel.subjectType) {
      if (this.channel.subjectType == 4) {
        //节目集 支持点播(mip)视频类型
        defaultType = '4';
        contentTypeList = Constant.CONTENT_LIST_TYPES.filter(type => {
          return type.id == 4;
        });
      }
      if (this.channel.subjectType == 6) {
        //比赛场次 支持资讯和点播(mip)视频类型
        contentTypeList = Constant.CONTENT_LIST_TYPES.filter(type => {
          return type.id == 0 || type.id == 4;
        });
      }
    }
    return {
      infoFlow: Object.assign(this.channel, {}),
      title: '', //内容页标题
      totalTopListNum: 0,
      pageIndex: 1,
      dataTotal: 0,
      ...
      contentSubjectTypeList: Constant.INFOFLOW_SUBJECT_TYPE,
  ...
  watch: {
    'insertIndexForm.insertIndex': function(newVal) {
      if (newVal) this.flag_error = false;
    },
    insertFlag(val) {
    ...
    'ruleForm.program_column_id'(newVal, oldVal) {
      // this.ruleForm.id = '';
  ...
  computed: {
    ...
    //根据信息流类型展示标题
    getTitle() {
      let name = this.infoFlow.subjectName || this.infoFlow.infoFlowName;
      return name;
    }
  },
  mounted() {
    if (this.infoFlow.isFrom == 'topic' && Constant.getItemByValue(Constant.CONTENT_LIST_TYPES, 9).id == '9') {
  ...
  methods: {
    ...
    getChangeDownStatus(row) {
      return (
        (row.isTop == 1 &&
          (this.tableList[(row.$index || row.$totalIndex - 1) + 1] &&
            this.tableList[(row.$index || row.$totalIndex - 1) + 1].isTop != 0)) ||
        // (row.isTop == 0 && (this.pageIndex != 1 ? (this.pageIndex - 1) * this.pageSize : 0) + (row.$index || (row.$totalIndex - 1)) + 1 != this.dataTotal)
        (row.isTop == 0 && (row.$index || row.$totalIndex - 1) + 1 != this.dataTotal)
      );
    },
    getJumpStatus(row) {
      return (
        this.infoFlow.status == '1' &&
        row.newsStatus == 1 &&
        (row.contentType == '1' ||
          row.contentType == '2' ||
          row.contentType == '3' ||
      ...
      this.$refs[formName].validate(valid => {
        if (valid && !this.imgGroupFlag) {
          ...
          Promise.all(getImgWidthPromises).then(arr => {
            ...
            params.channelId = params.infoFlowId = this.infoFlow.infoFlowId; // 频道ID
            ...
            if (ruleForm.type == '15') {
              ...
              params.contentTitle = matchList.filter(match => {
                return match.matchId == ruleForm.matchId;
              })[0].matchName;
              //新增relationId
    ...
    insertIndexSure(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let params = {};
          params.channelId = this.infoFlow.infoFlowId;
          ...
          params.contentPosition =
            ...
            (this.insertIndexItem.$index || this.insertIndexItem.$totalIndex - 1) +
            1;
          ...
          params = this.$bus.deleteNullProperty(params);
          this.$ajax({
            url: DI.infoFlow.insertAppointIndex,
    ...
    editPit(item) {
      item.infoFlowId = this.infoFlow.infoFlowId;
    ...
    changeTop() {
      ...
      this.$refs.topConfirmForm.validate(valid => {
        if (valid) {
          ...
          this.$ajax({
            url: DI.infoFlow.changeTop,
            data: JSON.stringify({
              isTop: isTopAction,
              channelId: this.infoFlow.infoFlowId,
    ...
    setCoverRelData(result) {
      let tempList = (result.cover || '').split(';');
    ...
    queryList(pageNum) {
      ...
      this.tableList = [];
      let params = {};
      /*params.infoFlowId = */ params.channelId = this.infoFlow.infoFlowId;
      ...
      this.$ajax({
        url: DI.infoFlow.queryContentList,
        data: JSON.stringify(params),
        context: this,
        success: res => {
          let data = res.data || {};
    ...
    selectCompetition(competitionId) {
      ...
      this.matchList = [];
      if (competitionId) {
        this.competitionName = this.$parent.$refs.add.$refs.tags.$refs.labels.getName(
          competitionId,
          'competitionId',
          'competitionName',
          this.competitionList
    ...
    checkListDisabled(row) {
      if (this.dialogSelected.length < 2) {
        ...
        let tempArray = this.dialogSelected.filter(item => {
          return item.matchId == row.matchId;
        });
        if (tempArray.length) {
          return false;
        } else {
    ...
    addColSure(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let pms = {
            infoFlowId: this.infoFlow.infoFlowId, //频道id
    ...
    removeColumnSure() {
      ...
      this.$ajax({
        ...
        data: JSON.stringify({
          infoFlowId: this.infoFlow.infoFlowId,
    ...
    handleColumn() {
      ...
      this.$emit('input', this.infoFlow);
    },
    queryColumnList() {
      //栏目查询
      this.duringAjaxColType = true;
      this.$ajax({
        url: DI.topic.querySpecialChannelColumn,
        data: JSON.stringify({
          subjectId: this.infoFlow.infoFlowId
        }),
        context: this,
        success: res => {
          this.duringAjaxColType = false;
          if (res.retCode == '0') {
