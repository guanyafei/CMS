...
export default {
  ...
  computed: {
    ...
    infoStatusKey() {
      return Constant.getItemByValue(Constant.MEDIA_INFO_STATUS, this.row.status).key;
    },
    itemCover() {
  ...
  methods: {
    showPreview(row) {
      let { infoTypeItem, infoStatusKey } = this;
      let $parentList = this.$parent.$parent;
      if (infoStatusKey === 'refused' || infoStatusKey === 'approving') {
        $parentList.handleEditClick(row);
      } else {
        if (infoStatusKey !== 'published') {
