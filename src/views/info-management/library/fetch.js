...
export const fetchLibraryListAction = (vm, options) => {
  vm.$ajax({
    url: DI.infoLibrary.list,
    data: JSON.stringify(options.params),
    context: vm,
...
export const fetchChannelListAction = (vm, options) => {
  vm.$ajax({
    url: DI.infoLibrary.channelList,
    context: vm,
    ...
    success: res => {
      if (res.retCode == '0') {
        ...
        for (let index = 0, len = totalSubjectList.length; index < len; index++) {
          ...
          if (!subjectObject[subject.infoFlowId]) {
            subjectObject[subject.infoFlowId] = subject;
          }
        }
...
export const doPulishToChannelAction = (vm, options) => {
  vm.$ajax({
    url: DI.infoLibrary.publishChannel,
    context: vm,
...
export const doEditExpireTimeAction = (vm, options) => {
  vm.$ajax({
    url: DI.infoLibrary.editExpireTime,
    context: vm,
    loadingText: '正在修改时效时间，请稍候！',
    data: JSON.stringify(options.params),
    success: res => {
      if (res.retCode == '0') {
        options.row.expireTime = options.params.expireTime;
        vm.close();
      } else {
        vm.$message.error(res.retMsg);
      }
