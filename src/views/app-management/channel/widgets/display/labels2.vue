...
<template>
	<div>
		<!-- 标签类型 -->
		<sn-form-item label="标签类型" v-show="labelTypeProp.showFlag">
		...
		<!-- 标签 -->
		<sn-form-item class="match-item" :label="labelSearchProp.labelName?labelSearchProp.labelName:labelName+'标签'" v-show="tagType!=7 && tagType!=8 && tagType!=9">
	  	<sn-autocomplete
      ...
      this.getList('1');
		...
		methods: {
			addTags(list) {
	    		...
	    		if (flag) {
            this.tags.push(item)
            typeof this.callback === 'function' && typeof this.callback === 'function' && this.callback(this.tags);
          };
	    	});
			},
			getList(type) {
	    ...
	    selectSeason(seasonId) {
	    	...
	    	if (seasonId && flagIndex == -1 && this.tagType == 3) {
		    	this.tags.push({
		    		labelId: seasonId,
		    		labelName: this.getName(seasonId, 'seasonId', 'seasonName', this.seasonList),
		    		labelType: 5
          });
	    	}
	    ...
	    selectRound(roundId) { // 阶段/轮次
	    	...
	    	if (roundId && this.tagType == 4) {
		    	...
		    	if (roundId && flagIndex == -1) {
			    	this.tags.push({
			    		labelId: roundId,
			    		labelName: this.getName(roundId, 'stageRoundId', 'stageRoundName', this.roundList),
			    		labelType: 6
            });
		    	}
	    ...
	    selectGroup(groupId) { // 小组标签类型 12？
	    	...
	    	if (groupId && flagIndex == -1 && this.tagType == 3) {
		    	this.tags.push({
		    		labelId: groupId,
            labelName: this.getName(groupId, 'groupId', 'groupName', this.groupList),
		    		labelType: 12
          });
	    	}
	    },
	    selectMatch(matchId) {
            ...
            labelTime: this.getName(matchId, 'matchId', 'matchDateTime', this.matchList),
		    		labelType: 7
          });
          typeof this.callback === 'function' && this.callback(this.tags);
	    	}
	    },
	    getCompetitonList() {
	    	this.$ajax({
